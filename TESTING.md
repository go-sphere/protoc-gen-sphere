...
> **Prefer `buf build` over `protoc` for sourcing third-party deps.** Imports
> such as `google/api/*.proto` and `buf/validate/validate.proto` are **not** in
> any Go module, so a pure-`protoc` setup has to vendor them by hand. `buf build`
> avoids that entirely: it resolves every dependency from a `buf.lock`, bundles
> the imports into the output, and keeps source info on by default (so comments
> survive — no `--include_source_info` needed). protoc-gen-sphere uses this
> approach. Add a `buf.yaml` next to the fixtures listing the deps, run
> `buf dep update` to pin `buf.lock`, then compile each fixture with:
>
> ```sh
> buf build <testdata-dir> --path <testdata-dir>/proto/basic.proto \
>     --as-file-descriptor-set -o <testdata-dir>/pb/basic.pb
> ```
>
> `--as-file-descriptor-set` strips buf-only metadata so the output unmarshals
> straight into `descriptorpb.FileDescriptorSet`. Only `make testdata` needs
> `buf`; `go test` runs against the committed `*.pb` files. (If you must use
> `protoc`, vendor the missing `.proto` files into a committed `deps/` tree and
> pass `--include_imports --include_source_info`.)
...
7. **`protoc` drops comments unless you pass `--include_source_info`.** If the
   plugin emits anything derived from proto comments — Go doc comments, Swagger
   `@Description`, etc. — the descriptor set must be compiled with
   `--include_source_info`. Without it `protoc` omits `source_code_info` and the
   golden output silently loses every comment. (`buf build` keeps source info by
   default; `protoc` does not.)
